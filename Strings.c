...
// clear the string to contain nothing.
void clear(char *str) { str[0] = '\0'; }

// removes last character in string
void popback(char *str) { str[length(str) - 1] = '\0'; }
...
// will find first occurrence of substring in the main string and return -1 if
// it does not exist
...
// replaces a given amount of characters in a string with the characters in a
// provided substring
...
void insert(char *str1, const char *str2, int32_t index) {
  ...
  int32_t len2 = length(str2);
  char *temp =
      (char *)realloc(str1, sizeof(char) * (length(str1) + length(str2) + 1));
  if (!temp) {
    printf("Memory allocation failed :(\n");
    free(str1);
    exit(EXIT_FAILURE);
...
// removes a given amount of characters from the string indicated by the
// beginning and end parameters
